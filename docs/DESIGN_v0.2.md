...
| 工具註冊(單一份) | `cie/mcp_tools.py`:`register_tools(mcp, engine, *, include_writes)` + `do_*` 純函式邏輯;stdio 與 HTTP **共用一份**。HTTP 傳 `include_writes=False`(只掛讀工具),stdio 用預設 `True`(含寫工具 `log_calibration`)。 |
| 兩扇門 | **公開門(HTTP)= 唯讀**:只掛 query/predict/diagnose/method_swap,無 `log_calibration` → 網路無寫入路徑。**私有門(本機 stdio)= owner 唯一寫入**(`LOCAL_PRINCIPAL`,`can_write=True`)。 |
| 端點 | `POST /mcp`(streamable-http,stateless)、`GET /health`、`GET /`(public 狀態)。 |
| 雙 token 認證(§13.4) | `Authorization: Bearer <token>` **與** `?token=`/`?key=` query 皆可(claude.ai 網頁連接器只能用 query);常數時間比對(`hmac.compare_digest`);**未設密鑰 fail-closed → 401**。**HTTP 一切 token 皆唯讀**。 |
| CORS | 鎖 `https://([a-z0-9-]+\.)*claude\.ai`;`OPTIONS` 預檢與 401 皆帶 CORS 標頭(CORS 在 auth 外層)。 |
| 每請求身分 | pure-ASGI middleware 解析 token → reader `Principal`(`can_write=False`)設入 `contextvar`(stateless 保證 per-request 同 async task,工具可見);非 BaseHTTPMiddleware(免緩衝 SSE)。 |
| 設定 | `CIE_MCP_AUTH_TOKEN`(主要唯讀 token,日常 + 分享)/ `CIE_MCP_GUEST_TOKENS`(JSON `{token:label}` 或 `[token]`,額外唯讀 token,供個別撤銷)/ `CIE_MCP_HOST` / `CIE_MCP_PORT` / `CIE_MCP_STATELESS`。**無寫入 token**:owner 寫入只在本機 stdio,不需網路 token。 |
| 部署 | `Dockerfile`(uvicorn 啟動,host-agnostic:Fly/Railway/Render/VPS 皆可);輕量(stdlib+pydantic+REST,CF 後端免重 ML)。 |
| 測試 | `tests/test_mcp_gate.py`(兩扇門寫入閘 + 讀共享 + 機制硬分區 + token 解析)、`tests/test_mcp_http.py`(HTTP 只掛讀工具 / 雙 token / 401 / CORS / health / stdio 私有門全工具回歸);`tools/smoke_http.py`(真起 uvicorn + 真 MCP client 端到端:無寫入路徑)。 |

連接(claude.ai 新增自訂連接器):URL = `https://<host>/mcp?token=<CIE_MCP_AUTH_TOKEN>`(**唯讀** token;owner 寫入只在本機 Claude Code stdio,不需網路 token)。
...
## 16. 兩扇門:共享與寫入信任(接 remote MCP 前的設計依據)

本節定調 CIE 被多個對話 / 專案 / 人呼叫時的規則。核心一句:**兩扇門——公開門(HTTP)唯讀,日常與分享都走這;私有門(本機 stdio)= owner 唯一寫入。網路面永遠只讀不寫 → global 共享真相不可從網路污染;token 外洩最壞只是被讀。**
...
- **邊界**:輸出與推理品質是**客戶端模型**的事,伺服器不 babysit。CIE 負責的是「資料正確、機制不混、區間誠實、寫入只走可信門」這些**結構性**保證;弱客戶端轉述得好不好,不在伺服器職責內。

### 16.2 兩扇門:寫入只走本機 owner,網路面唯讀 — ✅ 已落地

不靠「猜誰在呼叫、開最強模型」,而是把寫入權**綁在門上**:

- **公開門(HTTP,`server_http.py`)= 唯讀。** 日常與分享都走這。HTTP 傳輸**只註冊讀工具**(query/recommend、predict、diagnose、method_swap);`log_calibration` 根本不掛 → 網路上**沒有任何寫入路徑**。所有 HTTP token 一律唯讀(無 owner-over-HTTP)。token 外洩最壞只是被讀。
- **私有門(本機 stdio,`mcp_server.py`)= owner 唯一寫入身分。** 靠「跑在你的機器上」授權,不經網路;刻意校正 session 才寫。
- **寫入完整性鐵則**(套用於 owner 自己,擋自己的失誤):
  - `A` 級寫入**必附 `protocol`**(人類感官真值來源,如 SCA_cupping),否則拒收(鐵則 §3/§5)。
  - `grade=prediction` 為內部保留級(引擎自身預測),**不得當人類真值注入**;`prediction` **不入真相、不進方向投票**(防 model collapse,既有保證未改)。

**實作(`cie/mcp_principal.py`):**
- **工具分門**:`register_tools(mcp, engine, *, include_writes)`。HTTP 傳 `include_writes=False`(只讀工具);stdio 用預設 `True`(含 `log_calibration`)。寫工具不在 HTTP 註冊 = **第一道**「網路無寫入路徑」。
- **`apply_write_trust(record, principal)` 的 `can_write` 閘 = 第二道(防禦縱深)**:`principal.can_write=False`(一切 HTTP reader)→ 拒收;即便日後誤把 write 工具掛上 HTTP,仍擋下。owner 過閘後再擋 `grade=prediction` 注入(擋失誤)。
- **A 須 protocol** 的最終裁決留在 `engine.log_calibration`(唯一一處),閘層不複刻,避免雙重邏輯漂移。
- owner 寫入仍**雙寫 canonical sink**(§15.1),嵌入器恆為 `workers_ai`(§14.5)。不再做 grade 上限 / 命名空間重導(HTTP 不寫,moot)。
- 測試:`tests/test_mcp_gate.py`(reader 不可寫、owner prediction 注入擋下、owner A 無 protocol 由 engine 擋下、owner 寫 self/global 皆放行、機制硬分區);`tests/test_mcp_http.py`(HTTP list_tools 只有讀工具)、`tools/smoke_http.py`([4] 公開門呼叫 log_calibration 被拒)。

### 16.3 共享:HTTP 唯讀共享 global,寫入只在本機 owner — ✅ 已落地

兩扇門下的共享規則(比舊版多租戶**更簡單也更安全**):

- **global 客觀層**(物理因果 + 策展語料 corpus/global.jsonl):**跨人共享、讀放開**,維護一份,所有客戶端受惠。
- **寫入**:只在**本機 stdio(owner)**。global 永遠寫不進網路面 → 共享真相不可被遠端污染。owner 在本機可寫 global 或自己的 self 層(刻意校正)。
- **HTTP 讀範圍**:公開門 reader 讀**共享真相**(global 客觀層 + owner 的 self 校準),`read_user_ids=None`(不過濾)。日常與分享見同一份;這是兩扇門接受的曝露面(token 外洩 = 被讀)。
- **per-tenant `self` 寫入 / 讀隔離 =「未來如需再加」**:若日後要讓訪客各自寫自己的 `self` 層並硬隔離彼此,加性讀過濾機制已就緒(見下),屆時再恢復寫入門 + 命名空間導向即可;本輪刻意不做(無網路寫入 = 無需多租戶寫入隔離)。
- 何時才該建兩個實例:需**硬性資料隔離**(隱私 / 法規)或**領域完全不同**時。否則單實例 + 兩扇門足矣。

**實作:**
- **讀範圍機制(就緒、預設關閉)**:`do_query` 把 `principal.read_user_ids` 當**加性過濾**下傳 `store.search(..., user_ids=...)`(memory/Qdrant 用 `MatchAny`,Vectorize 用 `$in`)。目前 reader 與 owner/stdio 皆 `None`(不過濾)。**加性、預設 None = 原行為**,不動任何檢索 / 收縮 / conformal 數學(鐵則:HTTP 層不重寫引擎邏輯)。`store.search(user_ids=[...])` 的過濾力仍有單測保護,供未來 self 隔離直接沿用。
- **token → 身分**:`CIE_MCP_AUTH_TOKEN` = 主要唯讀 token(日常 + 分享);`CIE_MCP_GUEST_TOKENS`(JSON `{token:label}` 或 `[token]`)= 額外唯讀 token(供個別發放 / 撤銷,label 僅作稽核標籤)。**皆解析為 reader(`can_write=False`)**;無 owner-over-HTTP。本機 stdio = `LOCAL_PRINCIPAL`(owner)。
- **stateless 為 load-bearing(`CIE_MCP_STATELESS=1`,啟動硬檢查)**:per-request reader principal 走 `contextvar`,須在 **stateless** streamable-http 下才保證被工具看到。**有狀態**模式會把工具派發跑進「該 session 首個請求建立的長壽任務」,後續請求中介層設的 principal 看不到 → 工具退回 contextvar 預設 = `LOCAL_PRINCIPAL`(owner、`can_write=True`)。HTTP 雖不掛寫工具(第一道),但「reader principal 確實抵達工具」是 `can_write=False` 防禦縱深的前提;故 `server_http.build_app` 對 `mcp_stateless=False` **fail-closed 拒啟動**(`RuntimeError`)。
- 測試:`tests/test_mcp_gate.py`(reader 讀共享 global、機制硬分區、`store.search(user_ids=)` 加性過濾仍有效);`tests/test_mcp_http.py`(HTTP 只掛讀工具、`build_app` 對 stateless=False 拒啟動、stdio 私有門掛全部工具且預設 owner)。
