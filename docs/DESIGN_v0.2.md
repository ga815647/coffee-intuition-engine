...
## 16. 共享、多租戶與寫入信任(接 remote MCP 前的設計依據)

本節定調 CIE 被多個對話 / 專案 / 人呼叫時的規則。核心一句:**MCP 給的是「知識隨取(讀)」,不是「判斷力自動上身」;要安全共享,讀放開、寫 global 鎖擁有者。**
...
- 附帶保護:CIE 的誠實輸出(區間/證據/警告)會框住弱客戶端——弱模型照轉述也較難亂講。**結構性保證 > 客戶端模型強度。**

### 16.2 寫入信任模型(防污染,不靠「開最強模型」)

防線做在**伺服器結構**,不靠呼叫端模型強度(無法保證誰呼叫、再強也會幻覺):

- `log_calibration` 把呼叫端送的資料**預設不可信** → 預設只給 `C`/`B`,**永不自動 `A`**。
- `A` 級寫入**必附 `protocol`**(人類感官真值來源,如 SCA_cupping),否則拒收(既有鐵則 §3/§5)。
- AI 自身輸出一律 `grade=prediction`,**不入真相、不進方向投票**(防 model collapse)。
- 最佳:`AI 提議` 與 `人類確認` 分兩步;成為 `A` 級 global 真相前需擁有者審核。
- 存取層加固(非取代上述):用 **token scope** 區分讀 / 寫工具;casual 讀者拿不到寫入工具。

### 16.3 多租戶:一個實例,user_id 命名空間(不要建兩個)

schema 既有的 `user_id` 分層就是多租戶基礎:

- **global 客觀層**(物理因果 + 策展語料 corpus/global.jsonl):**跨人共享、讀放開**,維護一份,所有客戶端受惠。
- **self 個人偏好層**:**按 user_id 硬隔離**,A 的口味不得污染 B 的推薦;檢索時個人層只取自己的 user_id。
- **寫入規則**:他人 / 他專案 → **讀共享 global + 寫只能進自己的 user_id**;**global 的寫入權只留給擁有者**(或經審核才入)。
- 何時才該建兩個實例:需**硬性資料隔離**(隱私 / 法規,他人資料完全不得碰你的庫)或**領域完全不同**時。否則多租戶優於雙實例(雙份 global 各自維護 = 更糟)。
- 前提:16.2 的寫入結構性防護是 16.3 安全共享的**前置條件**——沒有它,多租戶會把污染風險也變多租戶。
