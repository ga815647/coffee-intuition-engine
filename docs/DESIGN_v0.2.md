...
**為什麼換掉 Vectorize:** 個人規模(446 筆策展語料,上看數千)下,Vectorize 是「多一個要管理、要建索引、metadata filter 不回溯(§15.1 footgun)的外部有狀態服務」。而 446 筆 × 1024 維的 cosine kNN 在記憶體裡是毫秒級——**根本不需要外部向量 DB**。把向量庫收進行程記憶體後,整個雲端依賴只剩兩個無狀態 REST 呼叫(Workers AI 嵌入 + D1 query),部署面與失敗面都更小。
...
- **冷啟動延遲:** 首個請求需等重嵌 446 筆(數秒);可接受。要消除就上選用向量快照(已延後)。
