...
| 設定 | `CIE_MCP_AUTH_TOKEN`(你個人 member token,日常 claude.ai,寫自己的 self)/ `CIE_MCP_GUEST_TOKENS`(JSON `{token:user_id}` = member 命名空間,或 `[token]` = reader;保留字 global/self 拒收)/ `CIE_MCP_HOST` / `CIE_MCP_PORT` / `CIE_MCP_STATELESS`。**寫 global / 晉升不需網路 token**:只在本機 stdio。 |
| 部署 | `Dockerfile`(uvicorn 啟動,host-agnostic:Fly/Railway/Render/VPS 皆可);輕量(stdlib+pydantic+REST,CF 後端免重 ML)。 |
...
連接(claude.ai 新增自訂連接器):URL = `https://<host>/mcp?token=<CIE_MCP_AUTH_TOKEN>`(**member** token,寫入只落你自己的 self 客製層;寫 global / 晉升只在本機 Claude Code stdio,不需網路 token)。
...
### 14.7 生產上線形態(本輪定案):記憶體自幹 index + R2 共用 canonical + Workers AI 嵌入,跑在 Cloud Run — ✅ 已落地
...
**為什麼換掉 Vectorize:** 個人規模(446 筆策展語料,上看數千)下,Vectorize 是「多一個要管理、要建索引、metadata filter 不回溯(§15.1 footgun)的外部有狀態服務」。而 446 筆 × 1024 維的 cosine kNN 在記憶體裡是毫秒級——**根本不需要外部向量 DB**。把向量庫收進行程記憶體後,整個雲端依賴只剩兩個無狀態 REST 呼叫(Workers AI 嵌入 + R2 物件讀寫),部署面與失敗面都更小。

| 角色 | §14.1 原方案 | **本輪生產定案** |
|---|---|---|
| 向量庫 | Vectorize(外部有狀態) | **記憶體自幹索引**(`CIE_STORE_BACKEND=memory`;cosine kNN,冷啟動從 R2 重建) |
| canonical 真相 | R2 / D1(選配) | **R2(必備,單一共用真相)**(`config.canonical_backend` 為**衍生屬性**:有 CF 金鑰 + `CIE_R2_BUCKET` 即自動 `r2`;**無 `CIE_CANONICAL_BACKEND` 環境變數可設**) |
| 嵌入 | Workers AI bge-m3 | **同**(`CIE_EMBEDDING_PROVIDER=workers_ai`,1024 維) |
| 執行環境 | Worker / 任意容器 | **Cloud Run**(scale-to-zero `min-instances=0` → 閒置 $0;`max-instances=1` 保寫一致性) |

**架構:**
```
claude.ai ─► Cloud Run 容器(server_http.py:MCP + 雙 token 認證 + member 寫入閘,§16)
                 │  冷啟動一次:prime_serving_index ← R2 canonical(重嵌→記憶體索引)
                 │  每請求:記憶體 cosine kNN 召回(零外部呼叫)
                 ├─► Workers AI(REST,嵌入查詢 / 寫入時嵌入新記錄)
                 └─► R2(REST,canonical JSONL:讀=冷啟動載入,寫=member 校準同步落盤)
本機 stdio owner(mcp_server.py)── 同 memory + R2 + workers_ai、同一個 bucket ──► 寫 global / 晉升
```

**冷啟動生命週期(`prime_serving_index`,`cie/rebuild.py`):** 容器冷啟動時**一次性**從 R2 canonical 讀全量 → 用當前 Workers AI 嵌入器重嵌 → 灌進記憶體索引(446 筆約數秒、極少 neurons)。**同實例後續請求重用該索引**(非每請求重建);scale-to-zero 後下次冷啟動再建。只在「`memory` + `r2` + engine 有 canonical」三條件齊備才觸發,其餘組合回 `None`(離線開發退回灌 6 筆種子)。**可選的向量快照(把嵌好的向量也存 R2,冷啟動直接載入免重嵌)刻意延後**——「正確優先」,446 筆重嵌成本可忽略。

**member 寫入撐過 scale-to-zero(durability 命門):** 記憶體索引在 scale-to-zero 即丟,所以 `engine.log_calibration` **先 append R2 canonical、確認後才更新記憶體索引**(順序 load-bearing):「回 `ok=True` ⟺ R2 已落盤」。R2 寫入失敗會在 `store.upsert` 前拋例外 → 呼叫端不會收到假成功,也不會只寫進易失記憶體。member 的 self 校準因此撐過閒置縮容,下次冷啟動從 R2 重建回來。

**`maybe_get_canonical` 的關鍵調整:** 原本「記憶體後端有 `iter_records` → 不掛 sink(回 `None`)」對離線開發正確,但對生產**錯**:記憶體 `_canonical` payload 不跨行程,Cloud Run scale-to-zero 一縮就沒了。故新增第一條規則:**`canonical_backend == "r2"` 時一律掛 R2 sink,即便後端自帶 `iter_records`**。R2 是 owner(本機 stdio 寫 global)與 member(HTTP 寫 self)共用的單一真相 bucket。
...
- **鐵則全不動:** 機制硬分區、水非因果、A 須 protocol、prediction 不入真相、member 寫只落自有 ns——皆與後端無關,記憶體 / R2 / Vectorize 共用同一份 `cie.*` 引擎邏輯。
