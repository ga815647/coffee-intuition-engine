...
**結構(`eval/`):**
- `eval/dataset.jsonl`:留出集,完整 `Record`(含真值 `flavor`),目前 5 筆離線示例,涵蓋三機制(3 percolation / 1 immersion / 1 pressure)。這些豆**不得進主庫**。
- `eval/run.py` / `python -m eval.run`:每筆留出記錄 → 去 flavor → `engine.predict` → 比對:
  - **(a) MAE / RMSE**:各 L3 軸與 overall。
  - **(b) 覆蓋率**:真值落在預測 conformal 區間 `[lower, upper]` 的比例 vs 名目(~90%)。
  - **(c) 方向 / 排序**:**同機制**配對裡,預測高低排序與真值一致的比例(pairwise accuracy)——呼應「方向 > 絕對值」與 AUDIT A9「用配對判斷」。**跨機制不配對**(鐵則 §12.1)。
- 輸出:JSON 報告(`eval/report.json`,gitignored)+ 印表格。

**防洩漏(關鍵,三道):**
1. **留出豆絕不進召回庫**:用獨立記憶體 store 灌入**策展語料 `corpus/global.jsonl`(446 筆)並按內容指紋扣除 holdout**(豆+機制+泡法+核心參數;因語料不帶穩定 id,**不能靠 id 扣除**),**不是只灌 6 筆 seeds**。執行期再驗證 `holdout_ids` 與庫 id **互斥**(縱深防禦),且任何一筆 `evidence` 都不得是留出豆。eval 報告顯示 `庫=446`。
...
**期待值(務實):** 離線雜湊嵌入本就不準,harness **不對 MAE 下硬門檻**;測試只斷言「協定可跑、留出豆確被排除、覆蓋率與方向算得出、同機制配對、無洩漏、無寫回」。真實準度數字待接 `workers_ai` 嵌入 + 真實資料後,用**同一 harness** 直接複用。
