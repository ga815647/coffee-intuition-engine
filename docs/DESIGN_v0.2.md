...
- 測試:`tests/test_mcp_gate.py`(member 寫 global/他人 ns 被 confine 回自有、grade A→B、拒 prediction、流量上限、self 讀隔離、owner 晉升套 A-protocol + 預設留個人、reader 不可寫、機制硬分區);`tests/test_mcp_http.py`(HTTP 掛 member 寫但無晉升、stdio owner 門全 5 工具);`tools/smoke_http.py`([4] member 寫自有 ns 成功 / 寫 global 被 confine / global 未被污染 / self 讀互相隔離)。
