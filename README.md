...
#### 生產定案:Cloud Run + 記憶體自幹 index + R2 共用 canonical + Workers AI 嵌入($0 scale-to-zero)

本輪上線**不用 Vectorize**(降為選項):向量庫收進行程記憶體,冷啟動從 R2 canonical 重嵌重建(446 筆數秒);整個雲端依賴只剩 Workers AI(嵌入)+ R2(canonical)兩個無狀態 REST。詳見 DESIGN §14.7。
...
CIE_CF_ACCOUNT_ID / CIE_CF_API_TOKEN  # CF token 最小權限:Workers AI:Run + R2:Edit
CIE_R2_BUCKET=<bucket>              # → canonical_backend 自動為 r2(單一共用真相)
...
```bash
# 一次性:本機灌策展語料到 R2 canonical(需上面 CF / R2 金鑰在本機 .env)
python -m cie.bootstrap             # corpus/global.jsonl(446)→ R2 canonical
...
要點:`--max-instances=1`(R2 read-modify-write 單寫者一致性)、`--min-instances=0`(閒置 $0)、`--allow-unauthenticated`(認證在 MCP token 層,不靠 GCP IAM)。member 經 HTTP 寫的 self 校準**先落 R2 再更新記憶體**,撐過 scale-to-zero。本機 owner(stdio)用同一份設定 + 同一個 bucket 寫 global / 晉升,Cloud Run 下次冷啟動讀到。

## 設定(.env,見 .env.example)

開發預設全部離線可跑(記憶體向量庫 + local 雜湊嵌入)。後端與嵌入各自由環境變數選擇,缺金鑰一律自動退回離線:

- **向量庫**`CIE_STORE_BACKEND`:留空=自動(有 CF 金鑰+Vectorize→`vectorize`;有 Qdrant URL→`qdrant`;皆無→`memory`)。可強制 `memory|qdrant|vectorize`。**生產建議顯式設 `memory`**(記憶體自幹索引 + 冷啟動從 R2 重建;見上方 Cloud Run profile / DESIGN §14.7)。
...
離線開發的記憶體 / Qdrant 後端自帶 `_canonical`,不另寫此檔(`maybe_get_canonical` 偵測後略過)。**但生產記憶體後端 + R2(`canonical_backend=r2`)時,`maybe_get_canonical` 一律掛 R2 sink**:記憶體 `_canonical` 不跨行程,Cloud Run scale-to-zero 即丟,R2 才是 owner / member 共用的單一真相(見 DESIGN §14.7)。
