...
| `cie/snapshot.py` | **global → git 備份**:`python -m cie.snapshot`(匯出 global → `corpus/global.export.jsonl` 入公開 git + commit;誤刪可復原)/ `--restore`(upsert 回 D1、不動 self)。對 D1 唯讀;§15.3-A |
| `cie/export.py` | **全量私密備份**:`python -m cie.export`(global+self → `./backups/`,gitignored,含個資勿公開)。對 D1 唯讀;排程範本 `tools/backup_self.example.yml`;§15.3-B |
...
### 備份 / 復原(D1 誤刪也能救回 global;見 DESIGN §15.3)

D1 是單一共用真相。兩條互補匯出皆**對 D1 唯讀、確定性排序**:

```bash
# A. global → git(可公開、誤刪可一鍵復原)。curation 收尾跑一次 = 一個 commit。
python -m cie.snapshot            # 匯出 global → corpus/global.export.jsonl(入公開 git)+ commit
python -m cie.snapshot --restore  # 從上檔把 global upsert 回 D1(只還原 global,不動 self)
python -m cie.bootstrap --force --path corpus/global.export.jsonl   # 整庫災後重建(replace_all)

# B. 全量私密備份(含 self 個資,絕不入公開 git)
python -m cie.export             # global+self → ./backups/(gitignored);排程範本 tools/backup_self.example.yml
```

- `corpus/global.export.jsonl` = **完整 live global 快照**(策展種子 + 晉升 / global 寫入 / 修正)= 災後復原權威來源;與 `corpus/global.jsonl`(qa_merge 重生的策展種子)**分離不覆蓋**。
- `promote_customization` 成功會回 `snapshot_reminder` 提醒跑 snapshot(**git 副作用只在 CLI,工具不碰 git**)。
- §B 含各 guest 命名空間(個資)→ 送私密目的地(本機 / 私有 R2 / 私有 repo);**token 不入任何匯出**。
