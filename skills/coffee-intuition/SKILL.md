...
1. **指名載入優先** — 已知要哪個工具,一律用 `ToolSearch select:<完整工具名>`(如 `query_flavor_map`)**直接指名**,不靠關鍵字語意檢索。本工具描述是繁中,英文抽象詞(coffee/diagnose…)排不進前段。
2. **非得關鍵字搜時** — 用**繁中 + 確切參數/機制名**(`brew_mechanism`、immersion/percolation/pressure、「風味」);一次撈不到就**換語言/換詞再試**,別搜幾次就放棄。
