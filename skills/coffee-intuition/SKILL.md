...
## 撈取工具 / 失敗判讀(別把「搜不到」當「伺服器掛」)

CIE 四工具是**延遲載入**,要先撈進工具定義才能呼叫。撈取與判讀照這三條,免得重演 2026-06-22 的誤判(關鍵字搜不到 → 被錯讀成伺服器連不上、還腦補冷啟/授權過期):

1. **指名載入優先** — 已知要哪個工具,一律用 `ToolSearch select:<完整工具名>`(如 `query_flavor_map`)**直接指名**,不靠關鍵字語意檢索——指名最穩,與描述用詞 / 語言無關。
2. **非得關鍵字搜時** — 工具描述自 2026-06-24 起已補**中英雙語 keywords**,英文查詢命中率比舊版好(未必穩,仍以指名為先)。用**繁中或英文 + 確切參數 / 機制名**(`brew_mechanism`、immersion/percolation/pressure、「風味」/ flavor)起手;一次撈不到就**換語言 / 換詞再試**,別搜幾次就放棄。
3. **「tool_search 撈不到工具」≠「伺服器掛」** — 前者重搜/指名即解;判伺服器死活**必須有實際呼叫回的錯誤碼為憑**,**不得腦補** Cloud Run 冷啟動、授權過期、註冊表沒索引等未證原因。沒有錯誤碼就先重撈再呼叫。
