...
| `cie/embedding.py` | `Embedder` 介面;預設 `LocalHashEmbedder`(離線);雲端待接 |
| `cie/store.py` | `VectorStore`(Qdrant 記憶體/雲端);機制硬過濾召回 |
| `cie/retrieval.py` | `GRADE_WEIGHT`、`weighted_estimate`(收縮+區間)、`assess`(A級權重佔比→信心) |
| `cie/engine.py` | `Engine`:recommend/predict/diagnose/method_swap/log_calibration |
...
骨架 v0.2 可跑,9 測試全綠。開發預設全離線(Qdrant 記憶體 + 雜湊嵌入,免金鑰)。收縮與 conformal 為可運作的簡化版,標 `TODO(prod)`。
