...
| `cie/canonical.py` | canonical 真相層:`LocalJsonlCanonical` / `R2Canonical`;`maybe_get_canonical`(R2 強掛 sink) |
| `cie/rebuild.py` | 從 canonical 重嵌重建索引;`prime_serving_index`(冷啟動 memory←R2)|
| `cie/retrieval.py` | `GRADE_WEIGHT`、`weighted_estimate`(收縮+區間)、`assess`(A級權重佔比→信心) |
| `cie/engine.py` | `Engine`:recommend/predict/diagnose/method_swap/log_calibration(寫:先 R2 後記憶體) |
| `cie/seed.py` | 載入 `seeds/anchors.jsonl` A 級種子 |
| `mcp_server.py` | FastMCP stdio = **owner 門**:全 5 工具(query_flavor_map / log_calibration / predict_method_swap + 晉升 list_customizations / promote_customization);與 HTTP 共用 `cie/mcp_tools.py` |
| `server_http.py` | remote MCP(streamable-http)= **網路面**:member 受限寫(只落自有 self);身分 / 寫入閘在 `cie/mcp_principal.py`(三層 + 晉升,§16) |
...
**生產上線形態(已落地,見 `docs/DESIGN_v0.2.md` §14.7):** Cloud Run($0 scale-to-zero)+ **記憶體自幹索引**(`CIE_STORE_BACKEND=memory`,冷啟動從 R2 重嵌重建,**不用 Vectorize**)+ **R2 共用 canonical**(`canonical_backend` 衍生屬性,有 CF 金鑰 + `CIE_R2_BUCKET` 即自動 `r2`;owner 本機與 member HTTP 寫同一 bucket)+ **Workers AI bge-m3 嵌入**。member 寫入**先落 R2 再更新記憶體**(durability:撐過 scale-to-zero);`max-instances=1`(R2 單寫者一致)、`CIE_MCP_STATELESS=1`(隔離命門)。冷啟動載入在 `rebuild.prime_serving_index`,`server_http` / `mcp_server` 啟動時呼叫。測試 `tests/test_serving_index.py`。
...
- **持久化(生產定案改走「記憶體自幹 index + R2 共用 canonical」,Vectorize 降為選項)**:`store.py` 的 `VectorStore` 介面已含 **Vectorize 後端**(REST,保留可用),但**上線不用**——個人規模(446 筆)記憶體 cosine kNN 即毫秒級,免外部向量 DB。生產 `CIE_STORE_BACKEND=memory`,canonical 存 **R2**(單一共用真相),冷啟動 `rebuild.prime_serving_index` 從 R2 重嵌重建記憶體索引。驗收:重啟(Cloud Run scale-to-zero)後資料還在 + member 寫入撐過冷啟動(`tests/test_serving_index.py`)。詳見 §14.7。
